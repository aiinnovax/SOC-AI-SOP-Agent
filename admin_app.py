...
if menu == "Dashboard":
    ...
    with tab_create:
        ...
        if "admin_msg" in st.session_state:
            ...
            del st.session_state.admin_msg # Clear it so it doesn't show forever
            ...
            with st.form("create_user_form"):
                ...
                if st.form_submit_button("Create Account & Assign License", type="primary"):
                    if new_email and new_company:
                        # Strictly assign limit based on the selected plan
                        ...
                        # Save success message to session state and refresh
                        st.session_state.admin_msg = f"User Successfully Created! {new_company} has been assigned the {selected_plan} plan with a {sop_limit} SOP limit. Password: {new_password}"
                        st.rerun()
                    else:
                    ...
                    if st.form_submit_button("Save Changes", type="primary"):
                        for u in db["users"]:
                            if u["email"] == target_email:
                                u["plan"] = edit_plan
                                u["limit"] = PLANS[edit_plan] # Auto-updates the limit based on plan
